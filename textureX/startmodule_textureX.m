function [h] = startmodule_textureX(h,module)
%%
h.now = datestr(now,'yyyymmddHHMMSS');
diary(fullfile(h.project_path,'Log',[h.now,'_',module,'_textureX.xlog']))
...
moduleRead = read_textureX(h.project_path,module);
...
for entry = 1:num_extract %Loop over each entry
    ...
    if ~isempty(ind_moduleRead) || sum(ind_moduleRead) > 0
        ...
        %we want to compute based on the selected texture parameters
    ...
    if ~isempty(ind_missing) && sum(ind_missing) > 0
        ...
        test_missing = feval([module,'_textureX'],extractRead_entry,test_missing); 
...
diary off
disp(['TREX-RT>> Log file: ',h.now,'_',module,'_textureX.xlog'])
