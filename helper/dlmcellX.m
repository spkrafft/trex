function dlmcellX(file,cell_array,varargin)
%% <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><> %%
...
% Removed the check for cells within a cell. In this particular case, I
% just want to ignore that cell

% Added a strrep to remove any commas from any strings because it jacks up
% the alignment of the .csv file

% Changed defalut delimiter to comma
    ...
    for j = 1:size(cell_array,2)
        ...
        elseif iscell(cell_array{i,j})
         ...
         elseif ischar(cell_array{i,j})
             %Replace any , hanging around because it screws up alignment of
             %the csv
             output{i,j} = strrep(cell_array{i,j},',',' ');
         end;
