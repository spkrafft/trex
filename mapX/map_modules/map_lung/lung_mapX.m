function [test_missing] = lung_mapX(extractRead_entry,test_missing)
...
%%
        out = lung_map(map.I,X,Y,Z,...
                       [map.image_xpixdim,map.image_ypixdim,map.image_zpixdim]);
...
%%
        for count = 1:numel(ind)
            ...
            test_missing.map_file(ind(count)) = strcat('LUNG.MAP',parameter_str,map.roi_file);
...
%%
            ...
            disp(['TREX-RT>> Saving LUNGMAP: ',test_missing.map_file{ind(count)}])
