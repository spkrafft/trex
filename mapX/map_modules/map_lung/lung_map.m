function [map] = lung_map(varargin) 
...
%
%   LUNGS = LUNG_MAP(I,X,Y,Z)
%   
%   Parameters include:
%
%   Example
%   -------
%   [map] = lung_map(I,X,Y,Z)
% 
%   Class Support
%   -------------             
...
%%
[I,X,Y,Z,siz_vox] = ParseInputs(varargin{:});

%% Preallocation
stats = lung_features(0);
...
for count = 1:num_points
    ...
    tempstats = [];
    stats = lung_features(par_si{count},siz_vox);      
    for count_stats = 1:numel(names_stats)
...
%-----------------------------------------------------------------------------
function [I,X,Y,Z,siz_vox] = ParseInputs(varargin)
...
I = double(I);
if ndims(I) > 3
  error(message('images:lung_map:invalidSizeForI'))
end   
...
if nargin > 2
    ...
    siz_vox = varargin{5};
    validateattributes(siz_vox,{'numeric'},{'real','nonsparse','vector','size',[1,ndims(I)]},mfilename,'SIZ_VOX',5);
end

%%
clearvars -except I X Y Z siz_vox
