function [test_missing] = shape_mapX(extractRead_entry,test_missing)
...
for i = 1:num_missing
    if isempty(test_missing.map_file{i})
        ...
        disp('TREX-RT>> Starting SHAPEMAP')
...
%%
        out = shape_map(map.mask,X,Y,Z,abs(mean(diff(xV))),abs(mean(diff(yV))),abs(mean(diff(zV))));
...
%%
        for count = 1:numel(ind)
            ...
            test_missing.map_file(ind(count)) = strcat('SHAPE.MAP',parameter_str,map.roi_file);
...
%%
            ...
            disp(['TREX-RT>> Saving SHAPEMAP: ',test_missing.map_file{ind(count)}])
