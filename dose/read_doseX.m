function [moduleRead,filename] = read_doseX(project_path,module)
%%
moduleRead = [];
filename = [];
%%
...
%%   
    ...
    parameters = parameterfields_doseX([]);
    ...
    if strcmpi(module,'plan')
        for nameCount = 1:length(featureNames)    
            moduleRead.(['feature_',featureNames{nameCount}]) = cell(0);
        end
    else
        for nameCount = 1:length(featureNames)    
    ...
    if strcmpi(module,'dvh') || strcmpi(module,'dmh') || strcmpi(module,'spatialdvh') || strcmpi(module,'mapdvh')
        moduleRead.bins_dvh = cell(0);
        moduleRead.diff_dvh = cell(0);
        moduleRead.cumul_dvh = cell(0);
    end
end
