function [test_missing] = hist_mapX(extractRead_entry,test_missing)
...
%%
        for count = 1:numel(ind)
            ...
            test_missing.map_file(ind(count)) = strcat('HIST.MAP',parameter_str,map.roi_file);
...
%%
            ...
            disp(['TREX-RT>> Saving HISTMAP: ',test_missing.map_file{ind(count)}])
