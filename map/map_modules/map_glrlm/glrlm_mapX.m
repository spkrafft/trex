function [test_missing] = glrlm_mapX(extractRead_entry,test_missing)
...
for i = 1:num_missing
    if isempty(test_missing.map_file{i})
        ...
        disp('TREX-RT>> Starting GLRLMMAP')
...
%%
        for count = 1:numel(ind)
            ...
            test_missing.map_file(ind(count)) = strcat('GLRLM.MAP',parameter_str,map.roi_file);
...
%%
            ...
            disp(['TREX-RT>> Saving GLRLMMAP: ',test_missing.map_file{ind(count)}])
