function [test_missing] = ngtdm_mapX(extractRead_entry,test_missing)
...
for i = 1:num_missing
    if isempty(test_missing.map_file{i})
        ...
        disp('TREX-RT>> Starting NGTDMMAP')
...
%%
        out = ngtdm_map(map.I,X,Y,Z,...
                       'NumLevels',2^eval(map.parameter_bd),...
                       'GrayLimits',eval(map.parameter_gl),...
                       'Distance',eval(map.parameter_dist),...
                       'Dimension',map.parameter_dim);
...
%%
        for count = 1:numel(ind)
            ...
            test_missing.map_file(ind(count)) = strcat('NGTDM.MAP',parameter_str,map.roi_file);
...
%%
            ...
            disp(['TREX-RT>> Saving NGTDMMAP: ',test_missing.map_file{ind(count)}])
