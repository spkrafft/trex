function [h] = startmodule_mapX(h,module)
%%
h.now = datestr(now,'yyyymmddHHMMSS');
%diary(fullfile(h.project_path,'Log',[h.now,'_',module,'_mapX.xlog']))

start_tic = tic;
disp(['TREX-RT>> ',upper(module),' map started...'])
set(h.text_wait2,'String',[upper(module),' map started...'])
...
%% If both dimension and offset are parameters (i.e. GLCM) we combine the two, turn dim off
if sum(strcmpi(fields_param,'dim')) >= 1 && sum(strcmpi(fields_param,'offset')) >= 1
    offset = h.(module).offset;
    h.(module).offset = [h.(module).dim; offset];
    num.offset = numel(h.(module).offset); %num.offset + num.dim;
    
    fields_param = fields_param(~strcmpi(fields_param,'dim'));
    num = rmfield(num,'dim');    
end
...
moduleRead = read_mapX(h.project_path,module);
...
%%
    ...
    if ~isempty(ind_moduleRead) || sum(ind_moduleRead) > 0
        ...
        %we want to compute based on the selected map parameters
    ...
    if ~isempty(ind_missing) && sum(ind_missing) > 0
        ...
        test_missing = feval([module,'_mapX'],extractRead_entry,test_missing); 
...
%% Add moduleWrite to moduleRead...don't ignore previously calculated maps on moduleRead
[~,ind_diff] = setdiff(moduleRead.map_file,moduleWrite.map_file);

mNames = fieldnames(moduleRead);
for nCount = 1:numel(mNames)
    moduleWrite.(mNames{nCount}) = [moduleRead.(mNames{nCount})(ind_diff,:); moduleWrite.(mNames{nCount})];
end

%% Write out log data, module, and print some messages to console
log_filename = [h.now,'_',module,'_mapX.mat'];
...
%diary off
disp(['TREX-RT>> Log file: ',h.now,'_',module,'_mapX.xlog'])
