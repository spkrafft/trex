function [moduleVec] = write_doseX(moduleWrite,log_filename)
...
%%
if isfield(moduleWrite,'bins_dvh')
    moduleWrite = rmfield(moduleWrite,'bins_dvh');
    moduleWrite = rmfield(moduleWrite,'diff_dvh');
    moduleWrite = rmfield(moduleWrite,'cumul_dvh');
end
...
if strcmpi(module,'plan')
    moduleVec.feature_space = cell(num_entries,num_combinations*num_feat);
    
    %% Isolate just the feature space data into a single matrix for ease...
    feature_space = cell(num_entries*num_combinations,numel(fields_feat));
