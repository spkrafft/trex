function [stats,doseBins,volHist,cumVolHist] = dvh_features(varargin)
...
for i = 5:5:100
    ...
    if isempty(ind)
        ...
        stats.(strcat('rV',num2str(i))) = cumVolHist(ind)/stats.Volume;
    end
end
...
%     else
%         stats.(strcat('rVS',num2str(i))) = 1 - cumVolHist(ind)/stats.Volume;
%     end
% end
...
%     else
%         stats.(strcat('aVS',num2str(i))) = stats.Volume - cumVolHist(ind);
%     end
% end

%% Relative D5-100
for i = 5:5:100
    ind = find(cumVolHist/stats.Volume < i/100,1);
    if isempty(ind)
...
%% MOC5-100
for i = 5:5:100
    ind = find(cumVolHist/stats.Volume >= (100-i)/100);
    if isempty(ind)
...
%% MOH5-100
for i = 5:5:100
    ind = find(cumVolHist/stats.Volume <= i/100);
    if isempty(ind)
...
for i = 1:length(exponentn)
    ...
    stats.(name) = sum((doseBins.^(1/n)).*(volHist./stats.Volume))^(n);
    clear name n
end
